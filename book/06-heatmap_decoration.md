...
fraction weighted by the width of DMRs ($\sum{w \dot p}/\sum{w}$).
...
`HeatmapAnnotation()` only supports axis labels, but it is easy to implement it by decorations.
...
There are some other scenarios where decoration is helpful:

- When you have many annotations, you can add lines to separate them. See example in Section \@ref().
