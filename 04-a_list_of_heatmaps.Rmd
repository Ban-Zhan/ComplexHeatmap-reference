...
```{r heatmap_list_relative_size, fig.width = 10}
...
```{r heatmap_list_gap, fig.width = 10}
ht1 = Heatmap(mat1, name = "rnorm", col = col_rnorm)
ht2 = Heatmap(mat2, name = "runif", col = col_runif)
ht3 = Heatmap(le, name = "letters", col = col_letters)
ht_list = ht1 + ht2 + ht3
...
The value of `padding` should be a unit vector with length 4. The four values correspond to the space at
...
pushViewport(viewport(x = 0, w = 0.5, just = "left"))
draw(ht, padding = unit(c(2, 20, 2, 2), "mm"), column_title = "`padding` is adjusted")
popViewport()
```
