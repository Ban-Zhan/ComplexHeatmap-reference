...
name which is like a unique identifier for the heatmap, which is important when you have a list of heatmaps. 
In later chapters, you will find the heatmap name is used for setting the "main heatmap" and is used
for decoration of heatmaps. If the name is not assigned, an internal name is assigned to the 
heatmap in a form of `matrix_%d`. In following examples in this chapter, we give the name `mat` to
the heatmap (for which you will see the change of the legend in next example heatmap).

## Colors

For heatmap visualization, colors are the major representation of the data matrix. In
...
of breaks values and a vector of corresponding colors. `colorRamp2()` linearly interpolates colors in every
...
As you can see, the color mapping function exactly maps negative values to green and positive values to red,
even when the distribution of negative values and positive values are not centric to zero.
Also this color mapping function is not affected by outliers.
...
interpolated. But remember this method is not robust to outliers because the mapping starts from the minimal value
in the matrix and ends with the maximal value. Following
color mapping setting is identical to `colorRamp2(seq(min(mat), max(mat), length = 10), rev(rainbow(10)))`.
...
![plot of chunk discrete_character_matrix](figure/discrete_character_matrix-1.png)
...
proper color space is a little bit subjective and it depends on specific data and color schema.
...
color is already controlled by the heatmap color mapping, you can only set the `col`
parameter in `gpar()` to control the border of the heatmap grids.
...
set heatmap title either on the row or/and on the column. Note at a same time you can only put e.g.
...
![plot of chunk row_column_title](figure/row_column_title-1.png)
...
![plot of chunk row_column_title](figure/row_column_title-2.png)
...
![plot of chunk row_column_title](figure/row_column_title-3.png)
...
![plot of chunk title_rotation](figure/title_rotation-1.png)
...
![plot of chunk cluster_basic](figure/cluster_basic-1.png)

```r
Heatmap(mat, name = "mat", show_column_dend = FALSE)
```

![plot of chunk cluster_basic](figure/cluster_basic-2.png)

```r
Heatmap(mat, name = "mat", row_dend_side = "right", column_dend_side = "bottom")
```

![plot of chunk cluster_basic](figure/cluster_basic-3.png)

```r
Heatmap(mat, name = "mat", column_dend_height = unit(4, "cm"), row_dend_width = unit(4, "cm"))
```

![plot of chunk cluster_basic](figure/cluster_basic-4.png)
...
![plot of chunk cluster_distance](figure/cluster_distance-1.png)
...
![plot of chunk cluster_distance](figure/cluster_distance-2.png)
...
![plot of chunk cluster_distance](figure/cluster_distance-3.png)
...
![plot of chunk cluster_distance_advanced](figure/cluster_distance_advanced-1.png)

If there are proper distance methods, you can also cluster a character matrix.
...
![plot of chunk cluster_character_matrix](figure/cluster_character_matrix-1.png)
...
![plot of chunk cluster_method](figure/cluster_method-1.png)
...
![plot of chunk cluster_object](figure/cluster_object-1.png)
...
![plot of chunk cluster_object](figure/cluster_object-2.png)
...
visualization of the dendrogram. Note **ComplexHeatmap** only allows rendering on the dendrogram edges.
...
![plot of chunk cluster_dendextend](figure/cluster_dendextend-1.png)
...
![plot of chunk cluster_dendsort](figure/cluster_dendsort-1.png)
...
![plot of chunk manual_order](figure/manual_order-1.png)
...
Seriation is an interesting technique for ordering the matrix (see this interesting post: http://nicolas.kruchten.com/content/2018/02/seriation/). 
The powerful [**seriation**](https://cran.r-project.org/web/packages/seriation/index.html) package implements quite a lot of methods for seriation. Since it is easy to extract row orders and column ordes from the object returned by the core function `seriate()` from **seriation** package. 
They can be directly assigned to `row_order` and `column_order` to make the heatmap.

The first example demonstrates to directly apply `seriate()` on the matrix. Since the "BEA_TSP" only allows a non-negative
matrix, we change the matrix to `max(mat) - mat`.
...
![plot of chunk dimension_name](figure/dimension_name-1.png)

```r
Heatmap(mat, name = "mat", show_row_names = FALSE)
```

![plot of chunk dimension_name](figure/dimension_name-2.png)

```r
Heatmap(mat, name = "mat", row_names_gp = gpar(fontsize = 20))
```

![plot of chunk dimension_name](figure/dimension_name-3.png)

```r
Heatmap(mat, name = "mat", row_names_gp = gpar(col = c(rep("red", 10), rep("blue", 8))))
```

![plot of chunk dimension_name](figure/dimension_name-4.png)
...
```r
# use named vectors to correctly correspond row names and row labels
...
![plot of chunk k_means](figure/k_means-1.png)

```r
Heatmap(mat, name = "mat", column_km = 3)
```

![plot of chunk k_means](figure/k_means-2.png)

```r
Heatmap(mat, name = "mat", row_km = 2, column_km = 3)
```

![plot of chunk k_means](figure/k_means-3.png)

More generally, `row_split` or `column_split` can be set to a vector or a data frame where
different combinations of levels split the rows/columns in the heatmap. The order of
each slice can be controlled by `levels` of each variable in `split` (in this case, each variable
should be a factor). If all variables are characters, the default order is `unique(row_split)` or `unique(column_split)`.
...
![plot of chunk split](figure/split-1.png)

```r
Heatmap(mat, name = "mat", row_split = data.frame(rep(c("A", "B"), 9), rep(c("C", "D"), each = 9)))
```

![plot of chunk split](figure/split-2.png)
...
![plot of chunk split](figure/split-3.png)
...
Heatmap(mat, name = "mat", row_split = cbind(rep(c("A", "B"), 9), cl))
```
...
![plot of chunk pam](figure/pam-1.png)
...
![plot of chunk split_row_order](figure/split_row_order-1.png)

Character matrix can only be split by `row_split`/`column_split` argument.


```r
Heatmap(discrete_mat, name = "mat", col = 1:4, row_split = rep(letters[1:2], each = 5))
```

![plot of chunk split_discrete_matrix](figure/split_discrete_matrix-1.png)
...
![plot of chunk split_graphical_parameter](figure/split_graphical_parameter-1.png)

In the scenarios described above, if `row_km`/`column_km` is set or `row_split`/`column_split` is set as a vector or a data frame,
hierarchical clustering is first applied to each slice which generates `k` dendrograms, then a parent dendrogram
is generated based on the mean value of each slice. **The height of the parent dendrogram is adjusted
by adding the maximal height of the dendrograms in all children slices and the parent dendrogram is added on top of the children
...
![plot of chunk split_dendrogram](figure/split_dendrogram-1.png)
...
![plot of chunk split_dendrogram](figure/split_dendrogram-2.png)

If `row_split`/`column_split` is set to a number, there is no title drawn around the heatmap, while
if `row_km`/`column_km` is set or `row_split`/`column_split` is set to categorical variables, the title
will be in form of `level1,level2,...`. Since the title for row/column slices contain the categorical level information,
it can be controlled by "a title template".

The package supports three templates. The first one is by `sprintf()` where the `%s` is replaced by the corresponding level.
In following example, since all combinations of `split` are `A,C`, `A,D`, `B,C` and `B,D`, if `row_title` is set to `%s|%s`, the four row titles
will be `A|C`, `A|D`, `B|C`, `B|D`.
...
For the `sprintf()` template, you can only put `A,B,C,D` in the title. However, when making the heatmap, you might
want to put more meaningful text instead of the interval levels. Once you know how to correspond the text to the level,
you can add it by following two template methods.

In following two template methods, special marks are used to mark the R code which is executable (it is called variable interpolation
...
package and the second one is from **glue** package which you need to install it to support it.
...
Or if you known the final number of row slices, you can directly set a vector of titles to `row_title`.


```r
Heatmap(mat, name = "mat", row_split = split, row_title = paste0("slice", 1:4))
```

```
## Warning in if (is.na(row_title)) {: the condition has length > 1 and only
## the first element will be used
```

```
## Warning in if (row_title == "") {: the condition has length > 1 and only
## the first element will be used
```

![plot of chunk unnamed-chunk-28](figure/unnamed-chunk-28-1.png)

Since there are titles for every row/column slice, it is also possible to have a global row/column title
which covers all slices. In following code, actually the `row_title` used in `draw()` function is the row title
of the heatmap list, although in following example there is one heatmap. The `draw()` function and the heatmap list
...
![plot of chunk split_gap](figure/split_gap-1.png)

```r
Heatmap(mat, name = "mat", row_km = 3, row_gap = unit(c(2, 4), "mm"))
```

![plot of chunk split_gap](figure/split_gap-2.png)
...
![plot of chunk split_gap](figure/split_gap-3.png)
...
of PDF file. Rendering heatmaps as raster images will effectively reduce the file size while the plot
looks exactly the same for your screem or if you print it out. In
...
composed by an array of rectangles (it is called "_cells_" here) with different filled colors. However, 
it is also possible to add more graphics or symbols for additional layers on the heatmap. There are two
arguments `cell_fun` and `layer_fun` which both are user-defined functions. 

`cell_fun` draws in each cell repeatedly, which is interally executed in two nested `for` loops, while
`layer_fun` is the vectorized version of `cell_fun`. `cell_fun` is easier to understand but `layer_fun`
is faster to execute.

`cell_fun` expects a function with 7 arguments (the argument names can be different from following, but the argumetn order
must be the same), which are:
...
The most common use is to add numeric values onto the heatmap:
...
![plot of chunk cell_fun](figure/cell_fun-1.png)
...
`pindex()` function from ComplexHeatmap which is like pairwise indexing. See follow example:
...
## a NA  NA  NA  "W" "B" "B" "B" NA  NA  NA  NA  NA  NA  NA  NA  NA  NA  NA 
## b "W" "W" "W" "B" "W" "B" "B" "B" "B" "B" "B" "W" "W" "B" "B" "W" "B" NA 
## c "B" "B" "W" "W" "B" "W" NA  "W" "B" "W" "W" "B" "B" "B" "B" "B" "W" "B"
## d "B" "W" "B" "W" "B" "W" "W" "W" "W" "B" "W" "B" "B" "B" "W" "W" "B" "B"
## e NA  "B" "B" "W" "B" "B" "W" "B" "B" "B" "B" "W" "W" "W" "W" "B" "B" "W"
## f NA  NA  NA  "B" "W" "W" NA  "W" "W" "W" "W" "W" "B" "W" "B" "W" "W" "W"
## g NA  NA  "B" "B" "B" "B" "W" "W" NA  "W" "B" "W" "B" "W" "B" "B" NA  "W"
## h "B" "B" "B" "W" "B" "W" "B" "B" "B" "B" "B" "B" "W" "W" "W" "B" "B" "B"
## i "B" "W" "B" "W" "W" "W" "W" "W" "W" "W" "B" "B" "B" "B" NA  "W" "B" NA 
## j "W" "W" "W" NA  "W" "W" NA  "W" "W" NA  "B" NA  NA  "B" "W" "W" "B" "W"
## k NA  NA  NA  NA  NA  NA  NA  "B" "W" "B" NA  NA  NA  "B" "B" "W" "W" "B"
## l NA  NA  NA  NA  NA  NA  "W" "W" "B" "B" "B" NA  "B" "W" "W" "W" "W" "W"
## m NA  NA  NA  NA  NA  NA  NA  "W" "W" "W" "B" NA  NA  "B" "B" "W" "W" NA 
## n NA  NA  NA  NA  NA  NA  "W" "B" "W" "W" "B" NA  NA  "B" "B" "B" "B" "W"
## o NA  "W" NA  "W" "W" "W" "B" "B" "W" "B" NA  "W" "B" "B" "B" "W" "W" "B"
## p NA  NA  NA  "W" "W" "B" "W" "B" "B" NA  "B" "B" "W" "B" "B" "B" "W" "W"
## q NA  NA  "B" "W" "B" "B" "B" "B" NA  NA  "B" "W" "B" "W" "W" "W" "W" NA 
## r NA  NA  "B" "W" "W" "B" NA  NA  NA  NA  "W" "B" "B" "B" "W" "B" "W" NA 
## s NA  NA  NA  NA  "W" "W" "B" NA  NA  NA  NA  NA  "B" "W" "W" "W" NA  NA 
##   s  
## a NA 
## b NA 
## c NA 
...
```
## Since all heatmaps/annotations have absolute units, the total width of the plot is 126mm mm.
## Since all heatmaps/annotations have absolute units, the total height of the plot is 104mm mm.
```
...
```
## Since all heatmaps/annotations have absolute units, the total width of the plot is 150mm mm.
## Since all heatmaps/annotations have absolute units, the total height of the plot is 136mm mm.
```
...
`Heatmap()` function actually is only a constructor, which means it only puts all the data and configuration
into the object in the `Heatmap` class. The clustering will only be performed after the `draw()` method is called. Under interactive mode (e.g.
the interactive R terminal where you can type your R code line by line), directly calling `Heatmap()` without returning
to any object prints the object and the print method (or the `show()` method) for the `Heatmap` class object calls `draw()` internally.
So if you type `Heatmap(...)` in your R terminal, it looks like it is a plotting function like `plot()`, you need to be aware that it is actually
not and in the following cases you will see nothing plotted.
...
The reason is in above three cases, the `show()` method will be called and thus `draw()` method is not called either. So,
to make the plot, you need to call `draw()` explicitly: `draw(Heatmap(...))` or you can first assign the heatmap to a object:
...
This scenario is also true for a heatmap list and `draw()` function accpets more arguments which e.g. controls the legends.
It will be discussed in later chapters. 


## get Orders and dendrograms 

The row/column orders of the heatmap can be obtained by `row_order()` and `column_order()` functions.
You can directly apply to the heatmap object returned by `Heatmap()` or to the object returned by `draw()`.
...
As explained in ..., `Heatmap()` function does not perform clustering, thus, when directly apply `row_order()` on `ht1`, clustering
will first be performed. Later when making the heatmap by `draw()`, the clustering will be applied again. This might be a problem
that if you set k-means clustering in the heatmap, since the clustering is applied twice, k-means might give you different clustering, 
which means, you might have different results from `row_order()` and the heatmap. `draw()` function also returns the heatmap which 
has been reordered, and apply `row_order()` on `ht2` just extract the row order, which makes sure the row order is exactly the same
as the one shown in the heatmap. So following is the recommended way to extract row/column orders:
...
Similar as `row_order()`/`column_order()`, the `row_dend()`/`column_dend()` return the dendrograms.
...
##   |  |--leaf "column2" ( value.column2 = -0.389, x = 0.5 )
##   |  `--leaf "column7" ( value.column7 = -0.229, x = 1.5 )
##   `--leaf "column6" ( value.column6 = 0.21, x = 2.5 )
## 
## $matrix_21$`2`
## --[dendrogram w/ 2 branches and 2 members at h = 2.14]
##   |--leaf "column5" ( value.column5 = -0.342, x = 0.5 )
##   `--leaf "column9" ( value.column9 = -0.248, x = 1.5 )
## 
## $matrix_21$`3`
## --[dendrogram w/ 2 branches and 4 members at h = 2.93]
...
##      |--leaf "column3" ( value.column3 = -0.943, x = 2.5 )
##      `--leaf "column4" ( value.column4 = -0.575, x = 3.5 )
```

`row_order()`, `column_order()`, `row_dend()` and `column_dend()` also work for a list of heatmaps, it will
be expalined in ...
