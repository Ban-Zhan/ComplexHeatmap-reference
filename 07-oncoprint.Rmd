...
can be made for all alteration types for all genes and all samples. However, when there are many
more alteration types, defining colors for all of them actually is not realistic (assuming you have
ten different alteration types). On the other hand, you might have a secondary classes of
alterations that you only want to visualize in the oncoPrint while not to separate by them in the
barplot annotations.

Consider following example where there are intronic/exion snvs and indels (let's assume one gene can
only have either snvs or indels). The different combinations of alterations are `snv`, `intronic
snv`, `exonic snv`, `indel`, `intronic indel` and `exonic indel`.
...
To distinguish `intronic snv` and `exonic snv`, we didn't encode them as alteration types such as
`intronic_snv` and `exonic_snv`. The way we encode in `m` is to separate them into two different classes
where `snv`/`indel` in the primary class and `intronic`/`exonic` is the secondary class. Now in `m`,
there are four different alteration types: `snv`, `indel`, `intronic` and `exonic`. Next we define 
`alter_fun` for the four alterations.
...
Note, for `intronic` and `exonic` which are the secondary class, we only draw simple symbols (dots
for `intronic` and diagonal lines for `exonic`) with no specific colors. Thus, the number of genes
or samples that have `intronic`/`exonic` events cannot be visualized in the barplot annotations.
